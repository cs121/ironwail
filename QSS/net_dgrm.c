...
static int SendMessageNext (qsocket_t *sock)
{
	...
	if (sock->sendMessageLength <= RELIABLE_MTU)
	{
...
qsocket_t *Datagram_GetAnyMessage(void)
{
	...
	for (net_landriverlevel = 0; net_landriverlevel < net_numlandrivers; net_landriverlevel++)
	{
		...
		if (!dfunc.initialized)
			continue;
		sock = dfunc.Listen(true);

		while(1)
		{
			length = dfunc.Read(dfunc.Listen(true), (byte *)&packetBuffer, NET_DATAGRAMSIZE, &addr);
			if (length == -1 || !length)
			{
				...
				for (net_landriverlevel = 0; net_landriverlevel < net_numlandrivers; net_landriverlevel++)
				{
					if (net_landrivers[net_landriverlevel].initialized)
					{
						if (dfunc.GetAddrFromName(net_masters[k].string, &addr) >= 0)
						{
							...
							dfunc.Write(dfunc.Listen(true), (byte*)str, strlen(str), &addr);
						}
						else
						{
							if (sv_reportheartbeats.value)
