...
// read buttons
	...
	movement = GetEdictFieldValue(host_client->edict, "movement");
	if (movement)
	{
		movement->vector[0] = move->forwardmove;
		movement->vector[1] = move->sidemove;
		movement->vector[2] = move->upmove;
	}
}
...
void SV_RunClients (void)
{
	...
	for (i=0, host_client = svs.clients ; i<svs.maxclients ; i++, host_client++)
	{
		...
		if (!host_client->netconnection)
		{
			...
			eval_t *ev = GetEdictFieldValue(host_client->edict, "movement");
