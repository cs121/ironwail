...
static void SV_Multicast(multicast_t to, float *org, int msg_entity);
...
static void PF_TraceToss(void)
{
	...
	val = GetEdictFieldValue(tossent, "gravity");
	if (val && val->_float)
		gravity = val->_float;
	else
		gravity = 1;
...
static void PF_sv_te_spike(void)
{
	...
	SV_Multicast(MULTICAST_PVS_U, org, 0);
}
...
static void PF_sv_te_explosion(void)
{
	...
	MSG_WriteCoord(&sv.datagram, org[1], sv.protocolflags);
	MSG_WriteCoord(&sv.datagram, org[2], sv.protocolflags);
	SV_Multicast(MULTICAST_PHS_U, org, 0);
}
...
static void PF_sv_te_lightning1(void)
{
	...
	SV_Multicast(MULTICAST_PHS_U, start, 0);
}
...
static void PF_sv_te_particlerain(void)
{
	...
	SV_Multicast (MULTICAST_ALL_U, NULL, 0);
}
...
static void PF_infokey(void)
{
	...
	else if (ent <= (unsigned int)svs.maxclients && svs.clients[ent-1].active)
	{
		...
		r = buf;
		if (!strcmp(key, "ip"))
			r = NET_QSocketGetAddressString(svs.clients[ent].netconnection);
...
static void PF_multicast_internal(qboolean reliable, byte *pvs)
{
	unsigned int i;
	int cluster;
	mleaf_t *playerleaf;
...
static void SV_Multicast(multicast_t to, float *org, int msg_entity)
{
	...
	case MULTICAST_ALL_U:
		SZ_Write(((to==MULTICAST_ALL_R)?&sv.reliable_datagram:&sv.datagram), sv.multicast.data, sv.multicast.cursize);
		break;
	case MULTICAST_PHS_R: //we don't support phs, that would require lots of pvs decompression+merging stuff, and many q1bsps have a LOT of leafs.
	case MULTICAST_PHS_U:
		SZ_Write(((to==MULTICAST_PHS_R)?&sv.reliable_datagram:&sv.datagram), sv.multicast.data, sv.multicast.cursize);
		break;
	case MULTICAST_PVS_R:
	case MULTICAST_PVS_U:
		PF_multicast_internal(to==MULTICAST_PVS_R, Mod_LeafPVS(Mod_PointInLeaf(org, sv.worldmodel), sv.worldmodel));
		break;
...
//	PR_CheckEmptyString (s);
	...
	for (i = 1; i < MAX_PARTICLETYPES; i++)
	{
		if (!sv.particle_precache[i])
		{
			...
			SV_Multicast(MULTICAST_INIT, NULL, 0);
