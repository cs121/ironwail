...
//
	...
	for (i=0 ; i<3 ; i++)
		...
		if (cl.protocol == PROTOCOL_NETQUAKE && !NET_QSocketGetProQuakeAngleHack(cls.netcon))	//spike -- proquake has 16bit client->server angles. this includes most public servers still using protocol 15. this does not break demos as its only client->server state.
