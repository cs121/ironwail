...
/*
 * Fog-volume rendering has been removed.
 * Froxel fog is the only supported volumetric fog path.
 * Keep these stubs so legacy call sites continue to link cleanly.
 */
...
void R_FogVol_InjectIntoGrid (froxel_grid_t *grid, const fog_volume_t *vols, int num) { (void)grid; (void)vols; (void)num; }
int R_FogVol_BindForFroxelBuild (void) { return 0; }
