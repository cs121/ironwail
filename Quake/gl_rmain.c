...
void GL_CreateFrameBuffers (void)
{
	...
	GLuint scene_colors[2] = {framebufs.scene.color_tex, framebufs.scene.velocity_tex};
	...
	if (framebufs.scene.samples > 1)
	{
		...
		GLuint resolved_colors[2] = {framebufs.resolved_scene.color_tex, framebufs.resolved_scene.velocity_tex};
		framebufs.resolved_scene.fbo = GL_CreateFBO (GL_TEXTURE_2D, resolved_colors, 2, 0, 0, "resolved scene fbo");
	}
	else
	{
...
void GL_PostProcess (void)
{
	...
	if (motion_samples < 0) motion_samples = 0;
	if (motion_samples > 32) motion_samples = 32;
	velocity_texture = 0;
...
void R_SetupView (void)
{
        ...
        if (r_prev_frame_valid)
        {
                ...
                r_framedata.delta_time = (float)(cl.time - r_prev_frame_time);
        }
        else
        {
                memcpy (r_framedata.prev_viewproj, r_prev_matviewproj, sizeof (r_prev_matviewproj));
