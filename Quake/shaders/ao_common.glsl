...
vec3 AO_ReconstructViewPos(mat4 invProj, vec2 uv, float depth, float reversedZ)
{
    ...
    // Avoid division by zero; sign-preserving to handle both conventions
    float w = (abs(view.w) > 1e-6) ? view.w : sign(view.w) * 1e-6;
    return view.xyz / w;
}
...
vec3 AO_ReconstructNormalFromDepth(sampler2D depthTex, mat4 invProj, ivec2 pixel, vec2 invScreen, float reversedZ)
{
    ...
    // Ensure normal points toward camera (View-Space: camera is at origin, geometry has negative Z)
    if (n.z > 0.0) n = -n;
