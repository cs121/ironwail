...
*/
// world.c -- world query functions (debugged & lightly optimized)
...
areanode_t* SV_CreateAreaNode (int depth, vec3_t mins, vec3_t maxs)
{
	...
	if (sv_numareanodes >= AREA_NODES)
		Sys_Error ("SV_CreateAreaNode: overflow (AREA_NODES=%d)", AREA_NODES); // BUGFIX: guard
...
void SV_ClearWorld (void)
{
	...
	sv_numareanodes = 0;
	if (!sv.worldmodel)
		Sys_Error ("SV_ClearWorld: sv.worldmodel is NULL"); // BUGFIX: safety
...
qboolean SV_BoxInPVS (vec3_t mins, vec3_t maxs, byte* pvs, mnode_t* node)
{
	...
	if (node->contents < 0)
	{
		leaf = (mleaf_t*)node;
		leafnum = (int)(leaf - sv.worldmodel->leafs - 1);
		return (qboolean)(pvs[leafnum >> 3] & (1 << (leafnum & 7)));
	}
