...
static void TexMgr_LoadLightmap (gltexture_t *glt, byte *data)
{
	...
	if (use_half)
	{
		glt->internal_format = GL_RGBA16F;
	        size_t pixel_count = (size_t)glt->width * glt->height;
	        size_t float_count = pixel_count * 4;
	        GLfloat *float_data = (GLfloat *) q_malloc(float_count * sizeof (*float_data));
	        if (!float_data)
	                Sys_Error ("TexMgr_LoadLightmap: out of memory on %" SDL_PRIu64 " bytes", (uint64_t)(float_count * sizeof (*float_data)));
	        for (size_t pixel = 0; pixel < pixel_count; ++pixel)
	        {
	                const byte *src = data + pixel * 4;
	                const byte r = (gl_lightmap_format == GL_BGRA) ? src[2] : src[0];
	                const byte g = src[1];
	                const byte b = (gl_lightmap_format == GL_BGRA) ? src[0] : src[2];
	                const byte a = src[3];
	                float rgba[4];

	                rgba[0] = r * (1.0f / 255.0f);
	                rgba[1] = g * (1.0f / 255.0f);
	                rgba[2] = b * (1.0f / 255.0f);
	                rgba[3] = a * (1.0f / 255.0f);

	                if (use_srgb)
	                {
	                        rgba[0] = TexMgr_SRGBToLinear (rgba[0]);
	                        rgba[1] = TexMgr_SRGBToLinear (rgba[1]);
	                        rgba[2] = TexMgr_SRGBToLinear (rgba[2]);
	                }

	                float_data[pixel * 4 + 0] = rgba[0];
	                float_data[pixel * 4 + 1] = rgba[1];
	                float_data[pixel * 4 + 2] = rgba[2];
	                float_data[pixel * 4 + 3] = rgba[3];
	        }
